...
class ActionPredictor(nn.Module):
	def __init__(self):
		...
		self.fc1 = nn.Linear(internal_width, internal_width)
		self.fc2 = nn.Linear(internal_width, internal_width)
		self.fc3 = nn.Linear(internal_width, 4)
	def forward(self, x, y):
		z = F.relu(self.fc1(x)) + F.relu(self.fc2(y))
		z = self.fc3(z)
		return z
...
class Compressor(nn.Module):
	def __init__(self):
		...
		self.fc = nn.Linear(width, internal_width)
	def forward(self, x):
		x = self.fc(x)
		return x
...
class ConvNet(nn.Module):
	def __init__(self):
		...
		self.fca = nn.Linear(width, internal_width)
	def forward(self, x):
...
z_init_policy = torch.randn((1, internal_width)).to(device), torch.randn((1, internal_width)).to(device)
z_init_future = torch.randn((1, internal_width)).to(device), torch.randn((1, internal_width)).to(device)

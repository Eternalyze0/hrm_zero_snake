...
# THRESHOLD = 0.01
CURIOSITY_SCALING_FACTOR = 1
CURIOSITY_ENABLED = False
PURE_CURIOSITY = False
RENDER_EVERY = 1 #20
...
class ReplayMemory:
	...
	def sample(self, batch_size):
		return random.sample(self.buffer, batch_size) #+ [self.buffer[-1]]
...
def train(model_path=None):
	...
	compressor_net = hrm_for_snake.Compressor().to(device)
	action_predictor = hrm_for_snake.ActionPredictor().to(device)

	action_predictor_net_optimizer = optim.AdamW(chain(action_predictor.parameters(), compressor_net.parameters()), lr=LR, betas=(0.95, 0.95))

	# initialize optimism in the face of uncertainty
	optimism_optimizer = optim.AdamW(policy_net.parameters(), lr=LR, betas=(0.95, 0.95))
	def optimism():
		optimism_optimizer.zero_grad()
		_, q_values = policy_net((torch.randn((1, internal_width)).to(device), torch.randn((1, internal_width)).to(device)), 
			torch.randn((BATCH_SIZE, 4, 20, 20)))
		target_q_values = torch.abs(torch.randn((BATCH_SIZE, 4))).to(device) * 1e10
		loss = nn.functional.mse_loss(q_values, target_q_values)
		loss.backward()
		optimism_optimizer.step()
	...
	for episode in range(EPISODES):
		...
		hrm_for_snake.z_init_policy = torch.randn((1, internal_width)).to(device), torch.randn((1, internal_width)).to(device)
		hrm_for_snake.z_init_target = torch.randn((1, internal_width)).to(device), torch.randn((1, internal_width)).to(device)
		hrm_for_snake.z_init_future = torch.randn((1, internal_width)).to(device), torch.randn((1, internal_width)).to(device)
		...
		for _ in range(1000):
			...
			# inject a bit of noise into the hidden state to prevent stagnation
			hrm_for_snake.z_init_policy = (hrm_for_snake.z_init_policy[0] + torch.randn((1, internal_width)).to(device) * 0.1, 
				hrm_for_snake.z_init_policy[1] + torch.randn((1, internal_width)).to(device) * 0.1 )
			hrm_for_snake.z_init_future = (hrm_for_snake.z_init_future[0] + torch.randn((1, internal_width)).to(device) * 0.1, 
				hrm_for_snake.z_init_future[1] + torch.randn((1, internal_width)).to(device) * 0.1 )
			...
			if CURIOSITY_ENABLED:
				...
				curiosity = torch.nn.functional.mse_loss(compressor_net(next_obs_tensor_p), compressor_net(next_obs_tensor.reshape((1, 1, 1600))))
			...
			future_memory.push((obs, next_obs, action))
			obs = next_obs
			total_reward += reward
			if CURIOSITY_ENABLED:
			...
			if len(memory) >= BATCH_SIZE:
				...
				if CURIOSITY_ENABLED:
					...
					obs_batch, next_obs_batch, action = zip(*future_batch)
					obs_batch = torch.tensor(np.stack(obs_batch), dtype=torch.float32, device=device)
					next_obs_batch = torch.tensor(np.stack(next_obs_batch), dtype=torch.float32, device=device)
					...
					for _ in range(N_supervision):
						...
						_, next_obs_tensor_p = future_net((hrm_for_snake.z_init_future[0],
							hrm_for_snake.z_init_future[1]), obs_tensor)
						compressor_net.eval()
						curiosity = torch.nn.functional.mse_loss(compressor_net(next_obs_tensor_p), compressor_net(next_obs_tensor.reshape((BATCH_SIZE, 1600))))
						...
						action_predictor_net_optimizer.zero_grad()
						compressor_net.train()
						action_p = action_predictor(compressor_net(obs_batch.reshape(BATCH_SIZE, 1600)), compressor_net(next_obs_batch.reshape(BATCH_SIZE, 1600)))
						# loss = torch.nn.functional.cross_entropy(action_p, torch.Tensor(action))
						loss = torch.nn.functional.cross_entropy(action_p, torch.tensor(action, dtype=torch.long, device=action_p.device))
						loss.backward()
						action_predictor_net_optimizer.step()
